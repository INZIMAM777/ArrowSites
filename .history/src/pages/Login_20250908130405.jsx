...
export const Login = () => {
  ...
  useEffect(() => {
    if (firebase.isLoggedIn) {
      ...
      if (firebase.admin && firebase.admin.some(admin => admin.email === email)) {
        navigate('/admin');
      } else {
        navigate('/');
      }
  ...
  }, [navigate, firebase, slides.length, email]);
  ...
  const googleSign = async () => {
    ...
    try {
      ...
      // After successful Google login, check if user is admin
      // Note: You might need to get the email from the Google user object
      // This would require additional implementation in your Firebase context
      if (firebase.admin && firebase.admin.some(admin => admin.email === email)) {
        navigate('/admin');
      }
    } catch (err) {
