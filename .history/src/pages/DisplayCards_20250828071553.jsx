...
export const DisplayCards = () => {
  ...
  // Map URL parameter values to actual property values
  ...
  const applyFiltersAndSearch = (propertiesToFilter, search, filterParams) => {
    ...
    if (filterParams.propertyType) {
      const mappedType = mapPropertyType(filterParams.propertyType);
      filtered = filtered.filter(
        (property) =>
          normalizeString(property.type) === normalizeString(mappedType)
      );
    }
    ...
    // Price filters - handle different price units (lakh, crore)
    if (filterParams.minPrice) {
      filtered = filtered.filter((property) => {
        let price = parseFloat(property.price);
        // Convert to lakh if needed for comparison
        if (property.price_units === 'crore') {
          price = price * 100; // Convert crore to lakh
        }
