...
export const AddPropr = ({ onClose, editData }) => {
    const { handleListing, updateProperty, getLists, presentId } = useFirebase();
    ...
    const handleChange = (e) => {
        ...
        // If property type changes, generate a new ID (only for new properties)
        if (name === 'type' && !editData) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSubmitError(`Failed to ${editData ? 'update' : 'add'} property. Please try again.`);
        } finally {
            ...
            <div style={styles.flexRow}>
                <div style={{ ...styles.formGroup, ...styles.flexItem }}>
                    ...
                    <input
                        ...
                        name="id"
                        value={formData.id}
                        style={{...styles.input, backgroundColor: editData ? '#edf2f7' : '#f8fafc'}}
                        readOnly={!!editData}
                        placeholder="ID will be generated automatically"
                        onChange={editData ? undefined : handleChange}
                    />
                </div>
