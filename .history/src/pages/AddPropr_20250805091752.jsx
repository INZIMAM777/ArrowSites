...
export const AddPropr = () => {
  ...
  {/* Improved multi-select dropdown with better UX */}
  <div style={styles.amenitiesDropdownContainer}>
    <select
      multiple
      name="amenities"
    ...
    >
      {amenityOptions.map(amenity => (
        <option 
          key={amenity} 
          value={amenity}
          style={formData.amenities.includes(amenity) ? styles.selectedOption : {}}
        >
          {amenity}
        </option>
      ))}
    ...
    <div style={styles.selectHint}>
      ...
      {isMobile ? 'Tap to select multiple' : 'Ctrl/Cmd + click to select multiple'}
    </div>
  </div>

  {/* Selected amenities display */}
  {formData.amenities.length > 0 && (
    <div style={styles.selectedAmenitiesSection}>
      <h4 style={styles.selectedAmenitiesTitle}>Selected Amenities:</h4>
      <div style={styles.amenityTagContainer}>
        {formData.amenities.map(amenity => (
          <div key={amenity} style={styles.amenityTag}>
            {amenity}
            <button 
              onClick={() => handleAmenityChange(amenity)}
              style={styles.amenityRemoveButton}
