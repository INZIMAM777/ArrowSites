...
export const ArrowProvider = (props) => {
  ...
  const [presentIdd, setPresentIdd] = useState({})
const presentId = async () => {
  const snapshot = await getDocs(collection(firestore, "IdAll"));
  snapshot.forEach((docSnap) => {
    console.log(docSnap.id, docSnap.data());
    setPresentIdd(docSnap.data())
  });
};
 ...
 const properties = [
  ...
  return (<>
    <ArrowContext.Provider value={{
      ...
      error,firestore,presentId,presentIdd
    }}>
      {props.children}
