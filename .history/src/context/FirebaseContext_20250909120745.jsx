...
export const ArrowProvider = (props) => {
  ...
  useEffect(() => {
    if (user && admin.length > 0) {
      const found = admin.some((a) => a.email === user.email);
      setIsAdmin(found);
    } else {
      setIsAdmin(false);
    }
...
const presentId = async () => {
  const snapshot = await getDocs(collection(firestore, "properties"));
  const ids = [];
  snapshot.forEach((docSnap) => {
    // If you're using custom IDs, use docSnap.id
    // If you're storing ID in the document data, use docSnap.data().id
    ids.push(docSnap.id); // or docSnap.data().id depending on your approach
  });
  return ids;
};
  ...
  const handleListing = async (propertyData) => {
  try {
...
//   };
  ...
  // Add these functions to your Firebase context
const deleteProperty = async (id) => {
  try {
    await deleteDoc(doc(firestore, 'properties', id));
    getLists(); // Refresh the list
  } catch (error) {
...
const updateProperty = async (id, data) => {
  try {
    await updateDoc(doc(firestore, 'properties', id), data);
    getLists(); // Refresh the list
  } catch (error) {
  ...
  return (<>
    <ArrowContext.Provider value={{
      signUpEp, signInEp, signGoogle, removeSign, getLists, deleteProperty, updateProperty,isLoggedIn, properties, loading,
      error,firestore,presentId,admin,isAdmin,handleListing,user
    }}>
