...
const colorTheme = {
  dark: {
    ...
    text: {
      ...
      accent: "#3b82f6",    // blue accent in dark
    },
    border: {
      primary: "#334155",
      accent: "#3b82f6",
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        <img
          ...
          onError={(e) => { e.target.src = defaultPropertyImage; }}
        />

        {/* Badges */}
        <div style={{ position: "absolute", top: "12px", left: "12px", display: "flex", gap: "6px", flexWrap: "wrap" }}>
          {property.isFeatured && (
            <span style={{ backgroundColor: "#ef4444", color: "white", padding: "4px 10px", borderRadius: "50px", fontSize: "0.7rem", fontWeight: "700" }}>
              FEATURED
            </span>
          )}
          <span style={{ padding: "4px 10px", borderRadius: "50px", fontSize: "0.7rem", fontWeight: "700", backgroundColor: property.transaction_type === 'Rent' ? '#10b981' : '#3b82f6', color: "white" }}>
        ...
        {isLoggedIn && (
          <button
            ...
            style={{ position: "absolute", top: "12px", right: "12px", background: theme.background.overlayStrong || "rgba(255,255,255,0.95)", border: "none", borderRadius: "50%", width: "34px", height: "34px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}
          >
        ...
        {/* Price */}
        <div style={{ position: "absolute", bottom: "12px", left: "12px", background: isDarkMode ? "linear-gradient(135deg, #3b82f6, #1d4ed8)" : theme.button.primary, color: "white", padding: "6px 12px", borderRadius: "8px", fontWeight: "600", fontSize: "14px", display: "flex", alignItems: "center" }}>
      ...
      <div style={{ padding: "1.2rem" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.75rem" }}>
          <h3 style={{ fontSize: "1.1rem", fontWeight: "600", color: theme.text.primary, margin: 0, flex: 1, paddingRight: "0.5rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {property.title}
          </h3>
        ...
        {/* Features */}
        <div style={{ display: "flex", justifyContent: "space-between", padding: "0.75rem 0", borderTop: `1px solid ${theme.border.primary}`, marginBottom: "0.75rem" }}>
          {[ { icon: <FaBed />, value: property.bedrooms || 3, label: 'Beds' }, { icon: <FaBath />, value: property.bathrooms || 2, label: 'Baths' }, { icon: <FaRulerCombined />, value: property.area_sqft || 1200, label: 'Sqft' }, { icon: <FaHome />, value: property.year_built || 2020, label: 'Built' } ].map((item, idx) => (
        ...
        <NavLink to={`/Cards/${property.id}`} style={{ textDecoration: "none" }}>
          <button
            style={{ width: "100%", fontWeight: "600", border: "none", padding: "0.7rem", fontSize: "0.9rem", borderRadius: "8px", color: "white", background: isDarkMode ? "linear-gradient(135deg, #3b82f6, #1d4ed8)" : theme.button.primary, cursor: "pointer", transition: "all 0.3s ease" }}
            onMouseEnter={(e) => {
              e.currentTarget.style.background = theme.button.primaryHover;
              e.currentTarget.style.transform = "translateY(-2px)";
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.background = isDarkMode ? "linear-gradient(135deg, #3b82f6, #1d4ed8)" : theme.button.primary;
              e.currentTarget.style.transform = "none";
            }}
          >
            View Details
